...
| M5 | 打包（双平台）+ 边下边看 + 系统通知 + 设置页落地（**双限额、网卡绑定**） | ▶️ 本机打包已交付：**venv 包装 .app 路线**（scripts/make_app.sh，含图标/单实例/双击即开，无需 PyInstaller）+ Windows bat 启动脚本；正式对外分发再评估 PyInstaller |
...
| 侧栏 UI + 订阅生命周期（2026-09-09 补充） | UI 改为**左侧固定导航**（主页=今日限额+下载任务，订阅，下载=添加磁力/种子，设置在侧栏底部=限额上限/目录/全局限速/Cookie）；订阅删除改为**软删除**（「已删除」列表可恢复/彻底删除）；新增**集数追踪**（sub_episodes 表）+ 本地种子存档（data/torrents/）+ 每轮自动补拉——未下完的集数任务丢失后会自动重新入队续传，修复"删了订阅/丢了任务就永远不补下"的问题 |

| 镜像域名免疫（2026-09-09 补充） | 订阅只存**域名无关路径**（/RSS/…），拉取按镜像列表「最近可用优先」依次尝试并记住成功域名，单个域名失效自动切换；内置种子列表（tangbai.cc/mikanime.tv/mikanani.me/sakiko.de/kas.pub，2026-09 检索整理），设置页可增删与一键探测 |
| 番剧文件夹（2026-09-09 补充） | 订阅名自动取 RSS 标题并去「Mikan Project - 」前缀；每番独立目录 `<下载目录>/<番剧名>`（自动创建，持久化）；引擎 move_storage 支持——「整理」功能把旧订阅补名/补目录、把引擎任务文件搬进对应番剧文件夹（启动自动离线整理 + 订阅页手动整理可联网补标题） |
| 实时监视器（2026-09-10 补充） | 侧栏底部 Clash Verge 风格组件：↑↓ 速度迷你曲线（Canvas 每秒采样，数据取引擎任务实时速率）+ 应用进程内存/CPU 占用（psutil）；无动画循环，占用极低 |
| 全局停止开关（2026-09-13 补充） | 侧栏总开关 ⏸/▶：一键暂停/恢复所有下载与做种（引擎会话级暂停，状态持久化重启保持；暂停期间调度器跳过订阅检查；恢复后立即补拉放行） |
| 追更/做种可靠性（2026-09-13 修复，用户实测反馈） | ① 集数去重以**集数追踪为准**，不再被旧版 seen 标记卡死（老订阅的新集可正常下载，如尼古喵喵第 10 集）；② 集数种子下载失败自动**换镜像重试**并计入订阅异常状态（失败对用户可见）；③ 调度器**启动立即执行第一轮**（此前首轮要等一个间隔，用户设 4320 分钟即 3 天）；④ 集数恢复统一化：未完成的续传、**已完成过的重新入队做种**（重启后做种恢复）；任务列表删除=封存（不再自动恢复） |

当前进度：M3 的 RSS 订阅闭环已交付并经用户实测跑通；侧栏 UI、订阅生命周期、
镜像域名免疫、番剧文件夹与整理、实时监视器、**追更/做种可靠性修复**已交付
（119 项单测全绿）。剩余：番剧页注入订阅；UI 已相当完整，Vue 重构优先级降低。
