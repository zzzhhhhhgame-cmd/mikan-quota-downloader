...
**RSS 链接订阅（无需登录，定时自动追更）**、桌面壳与登录向导。84 项单测全绿；
真实引擎与真实 RSS 均已实机验证。剩余：Vue 正式前端、番剧页注入订阅。
...
- **订阅（RSS 链接，无需登录）**：粘贴任意 Mikan RSS 链接（如番剧页的
  ...
  自动检查新集（GUID 去重）；被 Cloudflare 拦截时按提示导入 Cookie（**无需登录账号**，
  浏览器能打开站点即可）；
...
- **会话向导**：「登录 Mikan → 我已登录完成」两步（弹真浏览器过 Cloudflare 后自动收割
  会话），另有手动 Cookie 导入兜底（订阅下载通常无需登录）；
