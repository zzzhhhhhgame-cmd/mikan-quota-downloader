...
| 页面 | 内容 |
|---|---|
| 主页 | 今日限额仪表盘（下载/做种进度条 + 上传闸门徽章）+ **下载任务表**（状态含「做种中」、进度、上下行速度、暂停/恢复/删除）+ 任务概览数字 |
| 订阅 | 添加 RSS 订阅（**自动提取番剧名**，如 `Mikan Project - 尼古喵喵` → `尼古喵喵`）、检查间隔设置、订阅管理（重命名/立即检查/停用/删除/整理）；「已删除」列表可恢复或彻底删除。每部番自动在下载目录下建「番剧名」文件夹，集数都下进去 |
| 下载 | 添加下载：磁力链接 / 种子文件（可边下边看、可指定本次目录） |
| 侧栏底部 · 实时监视 | Clash Verge 风格的速度曲线图（↑上传橙线 / ↓下载蓝线，每秒采样）+ 上传/下载速度 + 应用内存/CPU 占用 |
| 侧栏底部 · 实时监视 | **⏸ 全部停止 / ▶ 全部开始**：一键暂停/恢复所有下载与做种（引擎级暂停，重启保持；恢复后自动继续追更） |
| ⚙ 设置（侧栏底部） | 限额上限调整、默认下载目录、**Mikan 镜像站管理**、**全局限速**、站点 Cookie 导入 |
...
>
> **镜像域名免疫**：订阅只记录链接的「路径」部分（/RSS/…），拉取时按
> 「⚙ 设置 → Mikan 镜像站」列表依次尝试（最近可用优先）。所以不管你复制的是
> 哪个副站的链接、也不管哪个域名将来失效，都能自动切换，追更不断。
...
10. **镜像切换**：某镜像失效时拉取会自动换下一个域名（可用的优先）；你也可以在
    「⚙ 设置 → Mikan 镜像站」添加新域名（贴上 `https://…` 即可，会自动探测）、
    一键检测全部、或删除不用的。内置列表：tangbai.cc / mikanime.tv（官方新域名）/
    mikanani.me（官方主站）/ sakiko.de / kas.pub。
11. **番剧文件夹与整理**：订阅名称自动取自 RSS 标题（自动去掉「Mikan Project - 」前缀）；
    新订阅自动在下载目录下建「番剧名」文件夹，该番所有集数都下进去。
    「整理订阅与文件」按钮可把旧订阅/旧文件归位（补名称、补目录、把引擎里的任务
    文件搬进对应番剧文件夹，搬完自动重校验，不会丢数据）。
...
**在哪**：左侧「⚙ 设置」→「全局限速」卡片。填 KB/s（0 或留空 = 不限）→ 点「应用限速」。想**完全停止**传输请用侧栏的总开关（⏸）。
...
| 订阅全部显示「异常」且换镜像无效 | 所有镜像域名都失效/被墙 | 在「⚙ 设置 → Mikan 镜像站」添加新发现的镜像域名 |
...
>
> **👤 人工验证 #6：镜像域名自动替换（约 5 分钟）**
> 1. **在哪里**：重启应用 → 左侧「⚙ 设置」→「Mikan 镜像站」卡片。
> 2. **做什么**：点「一键检测」→ 查看各域名状态（可用/异常）；把输入框里的
>    `https://mikanani.kas.pub` 这类新域名添加进去。
> 3. **核心验证**：到「订阅」页面，故意粘贴一个**不同域名**前缀的同路径链接
>    （比如你用的是 tangbai.cc，这次贴 `https://mikanani.kas.pub/RSS/Bangumi?bangumiId=3992&subgroupid=370`）
>    → 会提示"订阅成功"而不是报错——同一条订阅只会保留一份（按路径去重），
>    拉取时自动落在可用域名上。
> 4. 判断成功：订阅状态绿色「正常」；已删除列表有旧订阅的话可顺便恢复/清理。
>    异常时把订阅行悬停的原因 + 终端最后 20 行发我。
